...
func _init(kart: Kart):
	...
	follower.loop = true
	follower.h_offset = randf_range(-64.0 * 0.8, 64.0 * 0.8)
	follower.progress_ratio = 0.0
	kart.position = follower.position
	follower.progress_ratio = 0.03

func process(karts: Array[Kart]) -> void:
	follower.progress = kart.path.curve.get_closest_offset(kart.position) + kart.path.curve.get_baked_length() * 0.01
